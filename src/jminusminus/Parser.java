...
public class Parser {
    ...
    private JExpression multiplicativeExpression() {
        int line = scanner.token().line();
        boolean more = true;
        JExpression lhs = moduleExpression();
        while (more) {
            if (have(STAR)) {
                lhs = new JMultiplyOp(line, lhs, moduleExpression());
            }else if(have(DIV)){
		lhs = new JDivitionOp(line, lhs, moduleExpression());
	    }else {
                more = false;
            }
        }
    ...
    private JExpression moduleExpression() {
        int line = scanner.token().line();
        boolean more = true;
